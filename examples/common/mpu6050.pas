...
type
  ...
  TMPU = object
    ...
    procedure DMPInitialize(Ai2c: TI2CMaster);
    ...
    procedure setMemoryBank(bank: byte; prefetchEnabled, userBank: boolean);
    ...
    procedure setGyroSampleRate(rate: byte);
  ...
  private
    fi2c: TI2cMaster;
    dmpPacketSize: uint16;
    procedure CheckReturnError(retval: Tesp_err; Msg: string);
  ...
  const
  MPU6050_ADDRESS_AD0_LOW = $68;
  MPU6050_ADDRESS_AD0_HIGH = $69;
  ...
  MPU6050_DMP_FIFO_RATE_DIVISOR  = 1;
...
const
  sWriteErr = 'WriteByte error';
  sReadErr = 'ReadByte error';
  ...
  dmpMemory: array[0..MPU6050_DMP_CODE_SIZE-1] of byte = (
	...
	$00, $00, $01, $00, $00, $44, $00, $01, $00, $05, $8B, $C1, $00, $00, $01, $00,
...
begin
  fi2c := Ai2c;
{  if not testI2cConnection then
    writeln('Error testing i2c connection to MPU6050')
  else
  begin
    writeln('MPU6050 ID confirmed.');
...
procedure TMPU.DMPInitialize(Ai2c: TI2CMaster);
var
  b: byte;
...
begin
	...
	//DEBUG_PRINTLN(F("\n\nResetting MPU6050..."));
	reset();
	vTaskDelay(100); //delay(30); // wait after reset
	...
	writeln('Checking hardware revision...');
	write('Revision @ user[16][6] = ');
  readMemoryByte(b);
  writeln(HexStr(b, 2));
	writeln('Resetting memory bank selection to 0...');
	setMemoryBank(0, false, false);

	// check OTP bank valid
	writeln('Reading OTP bank valid flag...');
  write('OTP bank is ');
  if getOTPBankValid then
	...
	resetI2CMaster();
	vTaskDelay(10); //delay(30); // wait after reset
	//delay(20);
 ...
 //
	...
	setGyroSampleRate(4); // 1khz / (1 + 4) = 200 Hz
 //
 ...
 //
	...
	if not(writeProgMemoryBlock(@dmpMemory[0], MPU6050_DMP_CODE_SIZE, 0, 0, true)) then
    exit; // Failed

	//DEBUG_PRINTLN(F("Success! DMP code written and verified."));

	//// Set the FIFO Rate Divisor int the DMP Firmware Memory
	dmpUpdate[0] := 0; dmpUpdate[1] := MPU6050_DMP_FIFO_RATE_DIVISOR;
	writeMemoryBlock(@dmpUpdate[0], 2, 2, $16, true); // Lets write the dmpUpdate data to the Firmware image, we have 2 bytes to write in bank 0x02 with the Offset 0x16
...
var
  ...
  ret: Tesp_err;
begin
  //CheckReturnError(fi2c.WriteByte(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_WHO_AM_I), sWriteErr);
  //CheckReturnError(fi2c.ReadByte(MPU6050_DEFAULT_ADDRESS, data), sReadErr);
  CheckReturnError(fi2c.ReadByteFromReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_WHO_AM_I, data), sReadErr);

  result := data = $68;
  if not result then writeln('Data returned from reading MPU6050_RA_WHO_AM_I: $', HexStr(data, 2));
end;
...
begin
  //CheckReturnError(fi2c.WriteByte(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_ACCEL_XOUT_H), sWriteErr);
  //CheckReturnError(fi2c.ReadBytes(MPU6050_DEFAULT_ADDRESS, @data[0], length(data)), sReadErr);
  CheckReturnError(fi2c.ReadBytesFromReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_ACCEL_XOUT_H, @data[0], length(data)), sReadErr);
...
var
  b: byte;
begin
  //CheckReturnError(fi2c.WriteByte(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_GYRO_CONFIG), sWriteErr);
  //CheckReturnError(fi2c.ReadByte(MPU6050_DEFAULT_ADDRESS, b), sReadErr);
  CheckReturnError(fi2c.ReadByteFromReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_GYRO_CONFIG, b), sReadErr);
...
var
  b: byte;
begin
  //CheckReturnError(fi2c.WriteByte(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_ACCEL_CONFIG), sWriteErr);
  //CheckReturnError(fi2c.ReadByte(MPU6050_DEFAULT_ADDRESS, b), sReadErr);
  CheckReturnError(fi2c.ReadByteFromReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_ACCEL_CONFIG, b), sReadErr);
...
begin
  //CheckReturnError(fi2c.WriteByte(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_XA_OFFS_H), sWriteErr);
  //CheckReturnError(fi2c.ReadBytes(MPU6050_DEFAULT_ADDRESS, @data[0], length(data)), sReadErr);
  CheckReturnError(fi2c.ReadBytesFromReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_XA_OFFS_H, @data[0], length(data)), sReadErr);
...
begin
  ...
  //CheckReturnError(fi2c.WriteByte(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_XA_OFFS_H), sWriteErr);
  //CheckReturnError(fi2c.WriteBytes(MPU6050_DEFAULT_ADDRESS, @data[0], length(data)), sWriteErr);
  CheckReturnError(fi2c.WriteBytesToReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_XA_OFFS_H, @data[0], length(data)), sWriteErr);
end;
...
var
  data: array[0..5] of byte;
begin
  //CheckReturnError(fi2c.WriteByte(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_XG_OFFS_USRH), sWriteErr);
  //CheckReturnError(fi2c.ReadBytes(MPU6050_DEFAULT_ADDRESS, @data[0], length(data)), sReadErr);
  CheckReturnError(fi2c.ReadBytesFromReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_XG_OFFS_USRH, @data[0], length(data)), sReadErr);
...
begin
  ...
  //CheckReturnError(fi2c.WriteByte(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_XG_OFFS_USRH), sWriteErr);
  //CheckReturnError(fi2c.WriteBytes(MPU6050_DEFAULT_ADDRESS, @data[0], length(data)), sWriteErr);
  CheckReturnError(fi2c.WriteBytesToReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_XG_OFFS_USRH, @data[0], length(data)), sWriteErr);
end;
...
begin
  fi2c.readByteFromReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_BANK_SEL, bank);
  userBank := (bank and $20) > 0;
  prefetchEnabled := (bank and $40) > 0;
  bank := bank and $1f;
end;

procedure TMPU.setMemoryBank(bank: byte; prefetchEnabled, userBank: boolean);
begin
  bank := bank and $1f;
  if userBank then
    bank := bank or $20;
  if prefetchEnabled then
    bank := bank or $40;
  fi2c.writeByteToReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_BANK_SEL, bank);
end;

procedure TMPU.getMemoryStartAddress(out memAddr: byte);
begin
  fi2c.ReadByteFromReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_MEM_START_ADDR, memAddr);
end;

procedure TMPU.setMemoryStartAddress(memAddr: byte);
begin
  fi2c.WriteByteToReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_MEM_START_ADDR, memAddr);
end;

procedure TMPU.readMemoryByte(out b: byte);
begin
  fi2c.readBytesFromReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_MEM_R_W, @b, 1);
end;

procedure TMPU.setIntEnabled(enabled: byte);
begin
  fi2c.writeByteToReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_INT_ENABLE, enabled);
end;
...
procedure TMPU.setGyroSampleRate(rate: byte);
begin
  fi2c.writeByteToReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_SMPLRT_DIV, rate);
end;
...
function TMPU.writeProgMemoryBlock(data: PByte; dataSize: uint16; bank,
  address: byte; verify: boolean): boolean;
begin
  result := writeMemoryBlock(data, dataSize, bank, address, verify);
end;

procedure TMPU.setSlaveAddress(num, address: byte);
begin
  if (num <= 3) then
    fi2c.writeByteToReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_I2C_SLV0_ADDR + num*3, address);
end;
...
begin
  fi2c.writeByteToReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_DMP_CFG_1, config);
end;

procedure TMPU.setDMPConfig2(config: byte);
begin
  fi2c.writeByteToReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_DMP_CFG_2, config);
end;
...
begin
  fi2c.writeByteToReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_MOT_THR, threshold);
end;

procedure TMPU.setZeroMotionDetectionThreshold(threshold: byte);
begin
  fi2c.writeByteToReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_ZRMOT_THR, threshold);
end;

procedure TMPU.setMotionDetectionDuration(duration: byte);
begin
  fi2c.writeByteToReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_MOT_DUR, duration);
end;

procedure TMPU.setZeroMotionDetectionDuration(duration: byte);
begin
  fi2c.writeByteToReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_ZRMOT_DUR, duration);
end;
...
begin
  fi2c.readByteFromReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_INT_STATUS, result);
end;

procedure TMPU.CheckReturnError(retval: Tesp_err; Msg: string);
begin
  if retval <> ESP_OK then
    writeln(msg, esp_err_to_name(retval));
end;
...
var
  b, mask: byte;
begin
  CheckReturnError(fi2c.ReadByteFromReg(MPU6050_DEFAULT_ADDRESS, regAddr, b), sReadErr);
  mask := ((1 shl len)-1) shl (bitstart - len + 1);
  b := (b and not(mask)) or (byte(data) shl (bitstart - len + 1));
  CheckReturnError(fi2c.WriteByteToReg(MPU6050_DEFAULT_ADDRESS, regAddr, b), sWriteErr);
end;
...
var
  mask: byte;
begin
  CheckReturnError(fi2c.ReadByteFromReg(MPU6050_DEFAULT_ADDRESS, regAddr, data), sReadErr);
  mask := ((1 shl len)-1) shl (bitstart - len + 1);
...
begin
  ...
  begin
    ...
    progBuffer := data + i;
    fi2c.writeBytesToReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_MEM_R_W, progBuffer, chunkSize);
    ...
    begin
      setMemoryBank(bank, false, false);
      setMemoryStartAddress(address);
      fi2c.readBytesFromReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_MEM_R_W, verifyBuffer, chunkSize);
...
begin
  ...
  begin
    ...
    fi2c.readBytesFromReg(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_MEM_R_W, data + i, chunkSize);
