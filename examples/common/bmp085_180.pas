...
const
  ...
  BMPMeasureP0 = $34;      // Initiate pressure conversion with oversampling = 0
...
begin
  ...
  if fi2c_master.WriteByteToReg(BMPxxxAddr, BMPControlAddr, BMPMeasureP0) <> ESP_OK then exit;
  vTaskDelay(1);
  // Note: extended bits XLSB not used when oversampling = 0
  if fi2c_master.ReadBytesFromReg(BMPxxxAddr, BMPOutMsbAddr, @buf[0], 2) <> ESP_OK then exit;
  UP := (word(buf[0]) shl 8) or buf[1];
